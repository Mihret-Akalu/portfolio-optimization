...
def simulate_portfolio(returns_df, weights):
    ...
    values.iloc[0] = 1.0
    weight_array = np.array([weights[asset] for asset in returns_df.columns])

    for i in range(1, len(returns_df)):
        daily_ret = np.dot(weight_array, returns_df.iloc[i].values)
...
# Run backtests
# --------------------------------------------------
strategy_vals = simulate_portfolio(returns, weights_strategy)
benchmark_vals = simulate_portfolio(returns, weights_benchmark)
...
metrics_df = pd.DataFrame([metrics_strategy, metrics_benchmark], index=["Strategy", "Benchmark"])
...
plt.savefig("reports/figures/backtest_cumulative.png", dpi=300, bbox_inches="tight")
print("Backtest cumulative plot saved to 'reports/figures/backtest_cumulative.png'")
